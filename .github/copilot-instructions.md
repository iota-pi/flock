...
- Data flow: TanStack Query caches items/metadata (queryClient/queryKeys/fetchItems in src/api/queries.ts). Mutations optimistically update query cache then invalidate; Zustand stores UI/auth state (src/state/authStore.ts, src/state/uiStore.ts), and items.ts holds models/migrations.
- Vault storage uses DynamoDB driver at src/vault/drivers/dynamo.ts; API procedures are defined under src/vault/trpc/routers with Zod schemas in src/vault/trpc/schemas.ts. Item migrations run on the client in src/state/migrations; server migrations live in src/vault/migrations.
...
- Build/deploy: yarn build (tsc then vite build), yarn deploy --stage <stage>, yarn analyse for bundle inspection.
...
- Integration changes: update both client (src/api/Vault.ts, src/api/VaultAPI.ts) and server tRPC routers (src/vault/trpc/routers/*); adjust shared types in src/vault/types.ts and refresh e2e coverage.
